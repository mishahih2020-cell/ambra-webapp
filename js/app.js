...
function cartDiscount(){
  ...
  if(!p) return 0;
  const base = STATE.cart.filter(i=>getProduct(i.productId).category===p.appliesTo).reduce((s,i)=>s+cartLine(i),0);
  return Math.round(base * p.discount);
}
function cartDeliveryFee(){ return cartSubtotal() > 3000 ? 0 : 300; }
...
function viewCheckout(){
  ...
  '<div class="content"><div style="display:flex;flex-direction:column;gap:20px;padding:16px 20px 0;">'+
    ...
    '<div style="display:flex;flex-direction:column;gap:8px;border-top:1px solid var(--border-soft);padding:14px 0 16px;">'+
      '<div class="summary-row"><span>Товары ('+cartCount()+')</span><span class="val">'+formatPrice(cartSubtotal())+'</span></div>'+
      (promo?'<div class="summary-row"><span>Скидка</span><span class="val" style="color:var(--success);">–'+formatPrice(cartDiscount())+'</span></div>':'')+
      '<div class="summary-row"><span>Доставка</span><span class="val">'+(cartDeliveryFee()===0?'бесплатно':formatPrice(cartDeliveryFee()))+'</span></div>'+
    '</div>'+
  '</div></div>'+
  '<div class="sticky-bar" style="flex-direction:column;align-items:stretch;gap:12px;">'+
    '<div style="display:flex;align-items:baseline;justify-content:space-between;"><span style="font-size:13px;color:var(--text-secondary);font-weight:600;">Итого к оплате</span><span class="serif" style="font-size:22px;color:var(--cream);">'+formatPrice(cartTotal())+'</span></div>'+
  ...
  return '<div class="content" style="display:flex;">'+
    '<div style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:22px;padding:32px;text-align:center;">'+
      ...
      '<div style="width:100%;background:var(--bg-elev);border:1px solid var(--border-soft);border-radius:16px;padding:16px 18px;display:flex;flex-direction:column;gap:10px;">'+
        ...
        '<div style="display:flex;justify-content:space-between;font-size:12px;"><span style="color:var(--text-tertiary);">Начислено бонусов</span><span style="color:var(--brass-soft);font-weight:700;">+'+(order?Math.round(order.total*0.07):0)+'</span></div>'+
      '</div>'+
...
function viewProfile(){
  ...
  '<div class="content"><div style="display:flex;flex-direction:column;gap:20px;padding:18px 20px 18px;">'+
    ...
    '<div class="loyalty-card">'+
      '<div style="display:flex;align-items:center;justify-content:space-between;"><div class="eyebrow" style="color:var(--brass-soft);">Уровень Gold</div><div style="font-size:11px;color:var(--text-secondary);">кэшбэк 7%</div></div>'+
      '<div class="serif" style="font-size:30px;color:var(--cream);">1 240 бонусов</div>'+
      '<div style="display:flex;flex-direction:column;gap:6px;"><div style="height:6px;border-radius:3px;background:#3A2E1A;overflow:hidden;"><div style="width:62%;height:100%;background:linear-gradient(90deg,var(--brass),var(--brass-soft));"></div></div><div style="font-size:11px;color:var(--text-secondary);">До уровня Platinum — 760 бонусов</div></div>'+
    '</div>'+
...
function viewPromotions(){
  ...
  '<div class="content"><div style="display:flex;flex-direction:column;gap:22px;padding:18px 20px;">'+
    '<div class="loyalty-card" style="margin:0;">'+
      ...
      '<div class="serif" style="font-size:30px;color:var(--cream);">1 240 бонусов</div>'+
      '<div class="loyalty-tiers"><div class="loyalty-tier">Silver<br>5%</div><div class="loyalty-tier active">Gold<br>7%</div><div class="loyalty-tier">Platinum<br>10%</div></div>'+
    '</div>'+
    '<div style="display:flex;flex-direction:column;gap:10px;">'+
...
function onGlobalClick(e){
  ...
  switch(action){
    ...
    case 'place-order': {
      ...
      const order = {
        id:newId, date:'сегодня', itemsCount:cartCount(), total:cartTotal(), status:'transit',
      ...
      STATE.cart = []; STATE.promo = null; saveState();
      haptic('success');
